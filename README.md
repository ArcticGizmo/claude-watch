...
Claude Watch reads Claude Code's own session files under `~/.claude/` and updates in near real time. Each session shows one of four states:
...
- **Auto-start** — launch Claude Watch automatically when you open your first session (and optionally close it after the last one ends).
