...
# prompt: recognise /afk and /history (optionally namespaced, e.g. /claude-watch:afk), start-anchored
...
if ($action -eq 'prompt') {
  ...
  $running = [bool](Get-Process claude-watch -ErrorAction SilentlyContinue)

  if ($cmd -eq 'afk') {
    if (-not $running) {
      $reason = "Claude Watch isn't running, so external notifications can't be toggled right now."
    } else {
    ...
    if (-not $running) {
      $reason = "Claude Watch isn't running, so the history panel can't be opened."
    } else {
      if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
      Set-Content -Path (Join-Path $dir "$sid.history") -Value $sid -NoNewline -Encoding ASCII
      $reason = "Opening the Claude Watch history panel for this session."
    }
  }
