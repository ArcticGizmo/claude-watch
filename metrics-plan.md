...
### 1c. Plugin hooks — `plugins/claude-watch/scripts/invoke.ps1`
...
- **Per-project breakdown** — group by `cwd`/project name: "claude-watch: 5 sessions, 2h 40m".
...
- **Option A needs almost no storage** — stats are computed on demand and held in memory. Add a
  **per-transcript cache** keyed by `path + length + mtime` so re-aggregation is incremental
  ...
  `%AppData%/ClaudeWatch/stats-cache.json` so a fresh launch is fast.
- **If we add Option B**, persist Running stretches as append-only newline JSON under
  `%AppData%/ClaudeWatch/activity/` (e.g. one file per day, `{date}.jsonl`, records
